...
"""
from qtpy.QtWidgets import QGraphicsItem, QWidget, QGraphicsTextItem, QLineEdit
...
class QDMGraphicsNode(QGraphicsItem):
    ...
    def initSizes(self):
        ...
        self.title_horizontal_padding = 50
        self.title_vertical_padding = 0

    def initAssets(self):
        """Initialize ``QObjects`` like ``QColor``, ``QPen`` and ``QBrush``"""
        self._title_color = QColor("#91436b")
        self._title_font = QFont("Ubuntu", 15, 100)
        self._color = QColor("#a2a9c8")
        ...
        self._brush_title = QBrush(QColor("#a2a9c8"))
        self._brush_background = QBrush(QColor("#a2a9c8"))
    ...
    def initTitle(self):
        ...
        self.title_item.setPos(self.node.titlePosX + 9, self.node.titlePosY)
    ...
    def initContent(self):
        ...
        if self.content is not None:
            self.content.setGeometry(self.node.titlePosX + 9, self.node.titlePosY, 200, 30)
    ...
    def paint(self, painter, QStyleOptionGraphicsItem, widget=None):
        ...
        path_outline.addRoundedRect(-1, -1, self.width+2, self.height+2, self.edge_roundness + 15, self.edge_roundness + 15)
        painter.setBrush(Qt.NoBrush)
        painter.fillPath(path_outline, QColor("#a2a9c8"))
        if self.hovered:
            ...
            if not self.isSelected():
                painter.fillPath(path_outline, QColor("#b4c4c4"))
            else:
                painter.fillPath(path_outline, QColor("#d6a704"))
        else:
