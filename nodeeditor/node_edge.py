...
class Edge(Serializable):
    ...
    def __init__(self, scene:'Scene', edge_labels = [], start_socket:'Socket'=None, end_socket:'Socket'=None, nails = [], edge_type=EDGE_TYPE_DIRECT):
        """
        ...
        self.grEdge.setText(edge_labels)
        self.grEdge.setNails(self.nailList)
    ...
    def updatePositions(self):
        ...
        source_pos[0] += self.start_socket.node.grNode.pos().x() - 26
        source_pos[1] += self.start_socket.node.grNode.pos().y() - 9
        self.grEdge.setSource(*source_pos)
        if self.end_socket is not None:
            end_pos = self.end_socket.getSocketPosition()
            end_pos[0] += self.end_socket.node.grNode.pos().x() - 26
            end_pos[1] += self.end_socket.node.grNode.pos().y() - 9
