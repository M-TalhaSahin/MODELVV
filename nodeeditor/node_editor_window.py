...
class NodeEditorWindow(QMainWindow):
    ...
    def createActions(self):
        ...
        self.createTemplateActions()

    def createTemplateActions(self):
        self.actTemplatesList = []  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        for index in range(self.nodeeditor.tempList.__len__()):  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
            action = QAction('& ' + self.nodeeditor.tempList[index].name, self)
            action.triggered.connect(lambda checked, index=index: self.loadTemplate(index))
            self.actTemplatesList.append(action)  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

    def createMenus(self):
        """Create Menus for `File` and `Edit`"""
        self.createFileMenu()
        #self.createEditMenu()
        self.createTemplatesMenu()  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

    def createTemplatesMenu(self):  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        menubar = self.menuBar()  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        self.fileMenu = menubar.addMenu('&Templates')  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        for act in self.actTemplatesList:
            self.fileMenu.addAction(act)  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

    def loadTemplate(self, index):  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        self.nodeeditor.addNodes(index)  # +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    def setTitle(self):
        ...
        title = "UPPAAL Template Demonstrator - "
    ...
    def onFileOpen(self):
        ...
        if self.maybeSave():
            ...
            self.nodeeditor.updateNodeEdgeList(fname)
            self.nodeeditor.addNodes(0)
            self.menuBar().clear()
            self.createActions()  # were top
            self.createMenus()
